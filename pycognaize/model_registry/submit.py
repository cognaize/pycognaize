...
class ModelRegistry:
    ...
    def submit(self, run_id):
        ...
        postgres_engine = create_engine(f'postgresql://pycognaize:test@localhost:5432/pycognaize')

        with Session(postgres_engine) as session:
            experiment = Experiment(**kedro_metadata, metrics=metrics, **github_data)
    ...
    def _assert_all_changes_committed(self):
        try:
            status_process = subprocess.run(['git', 'status', '-s'], capture_output=True, check=True, text=True)

            status_output = status_process.stdout.strip()
        except FileNotFoundError:
            raise click.UsageError('Git should be installed to submit the data.')

        if status_output:
            raise click.UsageError('There are some modifications to the repo. Make sure all changes are committed.')
    ...
    def _get_kedro_metadata(self, run_id):
        ...
        with Session(sqlite_engine) as session:
            ...
            executed_date = datetime.datetime.strptime(run.blob['session_id'], '%Y-%m-%dT%H.%M.%S.%fZ')
            return {'executed_at': executed_date, 'command': run.blob['cli']['command_path']}
                ...
                for run_dir in metric_dir.iterdir():
                    if run_dir.is_dir() and run_dir.name == run_id:
                        ...
                        with file_path.open('r') as metric_file:
                            metrics[metric_dir_name] = json.loads(metric_file.read())
    ...
    def _get_github_data(self):
        github_data = {}
        try:
            repo_url_command = subprocess.run(['git', 'remote', 'get-url', 'origin'], capture_output=True, check=True,
                                              text=True)
            github_data['git_repo'] = repo_url_command.stdout.strip()

            username_command = subprocess.run(['git', 'config', 'user.name'], capture_output=True, check=True,
                                              text=True)
            github_data['github_name'] = username_command.stdout.strip()

            email_command = subprocess.run(['git', 'config', 'user.email'], capture_output=True, check=True,
                                           text=True)
            github_data['github_email'] = email_command.stdout.strip()

            commit_hash_command = subprocess.run(['git', 'rev-parse', 'HEAD'], capture_output=True, check=True,
                                                 text=True)
