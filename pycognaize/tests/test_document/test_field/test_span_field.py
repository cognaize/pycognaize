...
class TestSpanField(unittest.TestCase):
    ...
    def setUp(self):
        with open(RESOURCE_FOLDER + '/snapshots/64b1621a44acf9001017db4f/document.json') as document_json:
        ...
        self.raw_field_1 = self.data_with_group_key["output_fields"]["test_text_span__text_span"][0]
        self.raw_field_2 = self.data_with_group_key["output_fields"]["test_text_span__text_span"][2]

        self.page_10 = {10: create_dummy_page(page_n=self.raw_field_1['tags'][0]['page'])}
        self.page_8 = {8: create_dummy_page(page_n=self.raw_field_2['tags'][0]['page'])}

        self.span_field_1 = SpanField.construct_from_raw(raw=self.raw_field_1, pages=self.page_10, html=self.html)
        self.span_field_2 = SpanField.construct_from_raw(raw=self.raw_field_2, pages=self.page_8, html=self.html)
    ...
    def test__construct_from_raw(self):
        self.assertEqual(self.span_field_1.name, self.raw_field_1['name'])
        self.assertEqual(self.span_field_1.tags[0].raw_ocr_value, self.raw_field_1['tags'][0]['ocrValue'])
    ...
    def test_group_key(self):
        self.assertEqual(self.span_field_1.group_key, self.raw_field_1['groupKey'])
        self.assertEqual(self.span_field_2.group_key, self.raw_field_2['groupKey'])
